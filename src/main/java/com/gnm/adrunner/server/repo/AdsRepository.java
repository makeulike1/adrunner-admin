...
public interface AdsRepository extends CrudRepository<Ads, Integer> {
    ...
    @Query(value="select * from ads where redis_group=?1 and redis_db=?2 and (status=?3 or is_delete=true)", nativeQuery = true)
    public Integer findByRgAndRDB(Integer adsRedisGroup, Integer adsRedisDB, Integer status);
